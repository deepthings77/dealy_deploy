...
const useGetAllPost = () => {
    ...
    useEffect(() => {
        ...
        const fetchAllPost = async (currentPage, direction = "older") => {
            try {
                setLoading(true); 
                const res = await axios.get(`https://dealy-deploy.vercel.app/api/v1/post/all?page=${currentPage}&limit=15`, { withCredentials: true });
...
//                 const res = await axios.get('https://dealy-deploy.vercel.app/api/v1/post/all', { withCredentials: true });
